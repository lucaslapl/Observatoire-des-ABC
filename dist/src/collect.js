...
export function recomputeStatuses(db) {
    const YEAR = new Date().getFullYear();
    const ANNEE_MIN = 2000;
    const DUREE_ESTIME_TERMINE = 5;
    const all = db
        .prepare("SELECT id, statut, source, annee_debut, potentiellement_termine, potentiellement_en_cours, estime_termine FROM projets")
        .all();
    const snapAvancements = {};
    const snaps = db
    ...
    const upd = db.prepare("UPDATE projets SET statut = ?, potentiellement_termine = ?, potentiellement_en_cours = ?, estime_termine = ? WHERE id = ?");
    let changed = 0;
    let ptCount = 0;
    let pecCount = 0;
    let estimeCount = 0;
    ...
    for (const p of all) {
        ...
        let s = p.statut;
        if (hist.includes("Fini"))
            s = "termine";
        else if (p.source === "fondsvert-p113-2025")
            s = "a_venir";
        const anneeOk = p.annee_debut !== null && p.annee_debut >= ANNEE_MIN;
        const pt = s === "en_cours" && anneeOk && p.annee_debut <= YEAR - 3 ? 1 : 0;
        if (pt)
            ptCount++;
        const pec = s === "a_venir" && p.source !== "wayback" && anneeOk && p.annee_debut <= YEAR - 2 ? 1 : 0;
        if (pec)
            pecCount++;
...
export function printCollectSummary(s) {
