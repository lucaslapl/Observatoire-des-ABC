...
function haversineKm(lat1, lon1, lat2, lon2) {
    ...
    const a = Math.sin(t(lat2 - lat1) / 2) ** 2 +
...
export function computeAnomalies(db) {
    const rows = db
        .prepare("SELECT projet_id, code_geographique, lon, lat FROM communes WHERE lon IS NOT NULL AND lat IS NOT NULL")
        .all();
    const groupes = new Map();
    for (const r of rows) {
        if (!groupes.has(r.projet_id))
            groupes.set(r.projet_id, []);
        groupes.get(r.projet_id).push(r);
    }
    const upd = db.prepare("UPDATE communes SET anomalie = ?, distance_centre_km = ? WHERE projet_id = ? AND code_geographique = ?");
    let anomalies = 0;
    for (const [pid, cs] of groupes) {
        ...
        for (let pass = 0; pass < 2; pass++) {
            const outliers = cs.filter((c) => !flagged.has(c.code_geographique) && haversineKm(c.lat, c.lon, clat, clon) > DISTANCE_ANOMALIE_KM);
            if (outliers.length === 0)
                break;
            for (const o of outliers)
                flagged.add(o.code_geographique);
