import { z } from "zod";
import { getContribution, getLastAppliedAudit, getVerification, insertAudit, saveVerification, setContributionStatut, deleteVerification, } from "./db.js";
export const CONTRIBUTION_TYPES = ["statut", "note", "lien", "autre", "date_debut"];
...
export function verificationEtatPourStatut(s) {
    if (s === "termine")
        return "confirme_termine";
    if (s === "en_cours")
        return "confirme_en_cours";
    if (s === "va_debuter")
        return "toujours_a_venir";
    return null;
}
function payloadFromInput(c) {
    const p = {};
    if (c.statut_suggere)
        p.statut_suggere = c.statut_suggere;
    if (c.annee_debut_suggeree)
        p.annee_debut_suggeree = String(c.annee_debut_suggeree);
    if (c.annee_fin_suggeree)
        p.annee_fin_suggeree = String(c.annee_fin_suggeree);
    if (c.note)
        p.note = c.note;
    if (c.lien)
        p.lien = c.lien;
...
function verificationTarget(type, payload, avant) {
...
export function applyContribution(db, id, admin) {
    ...
    if (c.type === "date_debut") {
        ...
        if (Number.isInteger(debut) || fin !== null) {
            db.prepare("UPDATE projets SET annee_debut = ?, annee_fin = ?, updated_at = datetime('now') WHERE id = ?").run(Number.isInteger(debut) ? debut : null, fin, c.projet_id);
        }
    }
...
function projectStateOf(db, projetId) {
...
function applyState(db, projetId, s) {
    saveVerification(db, { projet_id: projetId, etat: s.etat, note: s.note, lien: s.lien });
}
export function rejectContribution(db, id, admin, noteAdmin) {
    const c = getContribution(db, id);
    if (!c)
        return { ok: false, error: "Contribution introuvable" };
    if (c.statut !== "en_attente")
...
export function revertContribution(db, id, admin) {
    const c = getContribution(db, id);
    if (!c)
        return { ok: false, error: "Contribution introuvable" };
    if (c.statut !== "validee")
    ...
    if (avant && ("annee_debut" in avant || "annee_fin" in avant)) {
        db.prepare("UPDATE projets SET annee_debut = ?, annee_fin = ?, updated_at = datetime('now') WHERE id = ?").run(avant.annee_debut ?? null, avant.annee_fin ?? null, c.projet_id);
    }
...
export function contributionToJson(c) {
