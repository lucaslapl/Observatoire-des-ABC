...
export async function collectFondsVert(db: DatabaseSync) {
  const specs = [
...
function isAbc(r: Record<string, string>): boolean {
  const hay = `${r["nom_du_projet"]} ${r["resume_du_projet"]} ${r["demarche"]}`;
  if (!hay) return false;
  if (/abc\s*terre/i.test(hay)) return false; // "ABC Terre" : carbone des sols, à exclure
