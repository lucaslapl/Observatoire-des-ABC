...
type GeoEntry = { lon: number; lat: number; name?: string };

export async function enrichGeocoding(db: DatabaseSync) {
  const rows = db
    .prepare(
      "SELECT DISTINCT code_geographique, libelle_geographique FROM communes WHERE code_geographique IS NOT NULL AND code_geographique != ''",
    )
    .all() as { code_geographique: string; libelle_geographique?: string }[];

  const cache: Record<string, GeoEntry> = fs.existsSync(GEO_CACHE)
    ...
    while (cursor < toFetch.length) {
      ...
      try {
        const res = await fetch(
          `https://geo.api.gouv.fr/communes/${r.code_geographique}?fields=centre,name`,
  ...
  const upd = db.prepare(
    "UPDATE communes SET lon = ?, lat = ? WHERE code_geographique = ?",
  );
  let updated = 0;
  for (const r of rows) {
